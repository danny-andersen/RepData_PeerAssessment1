...
```
    ...
    * The average number of steps per day is: `r format(avgStepsPerDay, nsmall=1, scientific = F)`  
    * The median number of steps per day is: `r format(medianStepsPerDay, nsmall=1, scientific = F)`
...
There are a number of missing values in the dataset - `r numNas`, which is equivalent to `r naPercent` % of the total dataset. One strategy to remove any missing values is to replace them with the average step count for the particular interval that is missing. 
...
To determine if there is any difference in activity patterns, determine if the activity was undertaken during the weekend or not:
...
The following two plots compare the weekend and weekday step activity as the average steps taken for each 5 minute period averaged across all measured days

```{r}
synthAvg <- filter(synthed, dayType == "Weekday") %>%
    group_by(interval) %>%
    summarise(avgWdSteps = mean(steps, na.rm = T)) %>%
    mutate(hm = ISOdatetime(1970,1,1,interval%/%60,interval%%60,0)) %>%
    mutate(hmc = strftime(hm,format="%H:%M"))
weAvg <- filter(synthed, dayType == "Weekend") %>%
    group_by(interval) %>%
    summarise(avgWeSteps = mean(steps, na.rm = T))
synthAvg$avgWeSteps <- weAvg$avgWeSteps
```
